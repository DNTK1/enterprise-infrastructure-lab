...
```bash
curl http://status.domena.lab/healthz
```

Informacje o aktualnie wdrożonej wersji:

```bash
curl http://status.domena.lab/api/status
```

Endpoint `/api/status` zwraca między innymi SHA commita oraz numer buildu Jenkinsa.