...
| Katalog | Co zawiera |
|---|---|
| [deploy-rocky.sh](deploy-rocky.sh) | skrypt tworzący nową VM Rocky Linux |
...
```bash
./deploy-rocky.sh <VMID> <nazwa> <RAM_MB> <vCPU> <dysk_GB> <VLAN> <PASSWORD>
```

Przykład:

```bash
./deploy-rocky.sh 160 rocky-test01 2048 2 30 20 Testpass1!
```
